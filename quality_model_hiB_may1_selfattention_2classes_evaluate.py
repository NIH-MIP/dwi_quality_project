...
def pick_dwi(row):
    if row["highb_path"] and os.path.exists(row["highb_path"]):
        return row["highb_path"]
    else:
        print(f"Skipping {row['patient_id']} - high B image not found.")
        skipped_cases.append({
            "patient_id": row["patient_id"],
            "patient_folder": row["patient_folder"],
            "expected_highb_path": row["highb_path"],
            "reason": "Missing or invalid highB image"
        })
        return None
...
if skipped_cases:
    ...
    skipped_df.to_csv(failed_images_file_hib, index=False)
    print(f"Skipped {len(skipped_cases)} cases with missing highB images. Saved to {failed_images_file_hib}")
...
train_dataset.save_failed_images(failed_images_file_hib)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(output_folder_path, f"hib_model_epoch_{epoch}_2class.pth"))
...
def generate_saliency_maps_from_csv(
    model,
    csv_path,
    image_dir,
    output_dir,
    ...
    model.eval()
    model.to(device)
    cam = GradCAM(nn_module=model, target_layers="conv_after_concat")

    patient_ids = pd.read_csv(csv_path)["patient_id"].tolist()
    for pid in patient_ids:
        folder = os.path.join(image_dir, pid)
        t2_path = os.path.join(folder, "T2.nii.gz")
        hib_path = os.path.join(folder, "hiB.nii.gz")

        if not (os.path.exists(t2_path) and os.path.exists(hib_path)):
            print(f"Skipping {pid} due to missing files")
            continue

        sample = {"t2_path": t2_path, "dwi_path": hib_path}
        sample = transform(sample)
        t2_tensor = sample["t2_path"].unsqueeze(0).to(device)
        hib_tensor = sample["dwi_path"].unsqueeze(0).to(device)

        t2_tensor.requires_grad_()
        hib_tensor.requires_grad_()
        output = model(hib_tensor, t2_tensor)
        pred_class = output.argmax(dim=1).item()
        saliency_maps = cam(class_idx=pred_class, inputs=(hib_tensor, t2_tensor))
        hib_map = saliency_maps[0][0].detach().cpu().numpy()
        t2_map = saliency_maps[1][0].detach().cpu().numpy()

        ref_img = nib.load(t2_path)
        nib.save(nib.Nifti1Image(sample["t2_path"].cpu().numpy().squeeze(), ref_img.affine),
                 os.path.join(output_dir, f"{pid}_T2_processed.nii.gz"))
        nib.save(nib.Nifti1Image(sample["dwi_path"].cpu().numpy().squeeze(), ref_img.affine),
                 os.path.join(output_dir, f"{pid}_hiB_processed.nii.gz"))
        nib.save(nib.Nifti1Image(t2_map, ref_img.affine),
                 os.path.join(output_dir, f"{pid}_T2_saliency.nii.gz"))
        nib.save(nib.Nifti1Image(hib_map, ref_img.affine),
                 os.path.join(output_dir, f"{pid}_hiB_saliency.nii.gz"))

        print(f"✅ Saliency maps saved for {pid}")
...
##torch.save(model.state_dict(), hiB_model_save_path.replace(".pth", "_2class.pth"))
##print(f"Final model saved to {hiB_model_save_path}")
# Load saved model checkpoint
checkpoint_path = r"/home/steckohj/image_quality_project/2025-05-01_13:13:53_hib_model/hib_model/hib_model_epoch_100_2class.pth"  # Update this path
model = DualInputDenseNet121(out_channels=num_prediction_classes).to(device)
...
generate_saliency_maps_from_csv(
    model=model,
    csv_path=csv_path,
    image_dir=image_dir,
    output_dir=output_dir,
    transform=valtest_transforms,  # same transform used during testing
    device=device
)
